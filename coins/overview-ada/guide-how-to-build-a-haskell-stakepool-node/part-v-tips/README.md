# PART V - TIPS

Part V includes the following topics containing additional procedures to simplify managing your stake pool:

* [Submitting a Simple Transaction](submitting-a-simple-transaction.md)
* [Transferring Files Using SSH](transferring-files-using-ssh.md)
* [Updating Configuration Files](updating-configuration-files.md)
* [Uploading Pool Metadata to GitHub](uploading-pool-metadata-to-github.md)
* [Obtaining a PoolTool API Key](obtaining-a-pooltool-api-key.md)
* [Configuring Glasgow Haskell Compiler Runtime System Options](configuring-runtime-options.md)
* [Setting TraceMempool](setting-tracemempool.md)
* [Increasing Swap File](increasing-swap-file.md)
* [Setting Up an External Passive Relay Node](setting-up-an-external-passive-relay-node.md)
* [Setting Up WireGuard](setting-up-wireguard.md)
* [Monitoring Node Security Using OSSEC Server and Slack](monitoring-node-security-using-ossec-server-and-slack.md)
* [Resetting an Installation](resetting-an-installation.md)
* [Fixing a Corrupt Blockchain](fixing-a-corrupt-blockchain.md)
* [Verifying an ITN Stake Pool](verifying-an-itn-stake-pool.md)
* [Fixing the Mnemonic Staking Balance Bug](fixing-the-mnemonic-staking-balance-bug.md)